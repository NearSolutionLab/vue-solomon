<template>
  <div class="lg:flex">
    <div class="md:ml-6 flex flex-col justify-center md:mt-0 mt-2">
      <h1 class="md:text-lg text-md">{{ t('solomon.title.order_pattern_analysis_report') }}</h1>
    ...
    <div class="flex flex-1 justify-end md:mt-0 mt-4">
      <div class="flex flex-col justify-center text-right">
        <span class="text-secondary">{{ t('solomon.text.recommended_batch_order_count') }}</span>
        <span class="text-2xl">{{ getHeaderData.orderCount }}</span>
      </div>

      <div class="flex flex-col justify-center text-right md:mx-16 mx-12">
        <span class="text-secondary">{{ t('solomon.text.order_count_per_sku') }}</span>
        <span class="text-2xl">{{ getHeaderData.orderCountPerSku }}</span>
      </div>
      <div class="flex flex-col justify-center text-right md:mr-10 mr-4">
        <span class="text-secondary">{{
          t('solomon.text.improvement_in_efficiency_compared_to_the_baseline')
        }}</span>
