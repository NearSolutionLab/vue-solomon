<template>
  <div class="lg:flex">
    <div class="md:ml-6 flex flex-col justify-center md:mt-0 mt-2">
      <h1 class="md:text-lg text-md">{{ t('solomon.title.client_name') }}</h1>
      <span class="text-secondary"> {{ getHeaderData.customerName }} </span>
    </div>
    <div class="flex flex-1 justify-end md:mt-0 mt-4">
      <div class="flex flex-col justify-center text-right">
        <span class="text-secondary">{{ t('solomon.text.rate_plan') }}</span>
        <span class="text-2xl">{{ t('solomon.text.pay_per_use') }}</span>
      </div>

      <div class="flex flex-col justify-center text-right md:mx-16 mx-12">
        <span class="text-secondary">{{ t('solomon.text.total_data_storage_count') }}</span>
        <span class="text-2xl">{{ getHeaderData.saveData }}</span>
      </div>
      <div class="flex flex-col justify-center text-right md:mr-10 mr-4">
        <span class="text-secondary">{{ t('solomon.text.total_data_usage_count') }}</span>
